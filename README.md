...
- **会话管理**：可以轻松地保存、重新打开、编辑和删除您的 SSH 会话；重新打开已连接的会话会直接激活对应标签页，不会重复创建连接。
...
- **主题系统**：提供 Ashell Light、Ashell Dark 和 VS Code Dark 三套主题；前两者采用黑白配色，VS Code Dark 使用蓝色活动强调。
- **默认字体**：UI 和终端默认使用系统已安装的 Noto Sans CJK SC 字体。
- **Windows 标题栏**：使用紧凑的自定义 Windows 风格标题栏，自动跟随当前主题，最小化、最大化和关闭按钮位于右侧。
