...
JShell is a Rust and GPUI desktop terminal workspace for local terminals, SSH sessions, SFTP file management, and remote text editing. The current source version is `1.0.1`; the UI follows the system language by default and can be changed in settings, while Chinese remains the default documentation entry.
