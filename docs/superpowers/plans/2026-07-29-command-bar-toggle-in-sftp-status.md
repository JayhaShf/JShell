...
以下规则用于收敛实施前审查发现的歧义，并作为后续评审与手动验证的验收条件：

1. `command_bar_toggle_tooltip_key` 保持在 `src/app/command_history.rs` 模块私有作用域；同文件的 `#[cfg(test)]` 子模块通过 `super::*` 调用，渲染函数必须使用该函数，避免测试与真实提示语逻辑分叉。
2. 两个状态栏分支都继续让状态或路径文本作为唯一可收缩区域（`flex_1 + min_w(0) + overflow_hidden`）；命令按钮、传输按钮和展开/折叠按钮保持固定自身宽度，不新增可换行文字或第二行。窄窗口手动验证以“文字允许截断、控制项不重叠、状态栏高度不变”为准。
3. `active_sftp` 缺失分支用于覆盖“当前工作区可渲染 SFTP 面板但句柄尚不可用”的状态；手动验证不依赖人为制造断开的 SSH 会话，而是分别检查该分支与正常 SFTP 句柄分支的结构和按钮可见性。
4. 单元测试只验证动态提示键选择；Debug 手动验证必须在简体中文和英文界面各检查一次打开与关闭后的工具提示，确认实际本地化文本分别切换。
